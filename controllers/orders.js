...
} = require("../middleware/generateInvoiceNumber");
...
const sendInvoiceToEmail = async (req, res) => {
  ...
  try {
    ...
    // Create PDF invoice - this returns the file path
    ...
    // Read the PDF file into a buffer for email attachment
    const pdfInvoice = fs.readFileSync(pdfFilePath);

    // Optional: Copy the file to your invoices directory if needed
    const invoicesDir = path.join(__dirname, "../invoices/email");
    ...
    const customInvoicePath = path.join(
      invoicesDir,
      `invoice-${orderData.customer.fullName}-${invoiceNumber}.pdf`
    );

    // Copy the generated PDF to your custom location
    fs.copyFileSync(pdfFilePath, customInvoicePath);

    // Send email with attachment
    await transporter.sendMail({
      from: process.env.EMAIL_USER,
      to: orderData.customer.email,
      subject: `Invoice for ${orderData.customer.fullName}, Order Number ${invoiceNumber}`,
      text: `Dear ${orderData.customer.fullName}, 
      ...
      attachments: [
        {
          filename: `invoice-${orderData.customer.fullName}-${invoiceNumber}.pdf`,
          content: pdfInvoice,
        },
      ],
    });
